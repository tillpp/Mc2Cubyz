...
const ListUnmanaged = @import("list.zig").ListUnmanaged;
...
pub const GarbageCollection = struct { // MARK: GarbageCollection
	...
	fn freeItemsFromList(list: *ListUnmanaged(FreeItem)) void {
		//std.debug.print("gc freeItemsFromList x: {d}\n", .{threadCycle});
		//std.debug.print("gc freeItemsFromList A: {d}\n", .{lists[threadCycle].items.len});
		//std.debug.print("gc freeItemsFromList B: {d}\n", .{list.items.len});
	...
	pub fn debug() void {
		//std.debug.print("gc list: {d}\n", .{lists[threadCycle].items.len});
	}
	pub fn removeThread() void {
		//std.debug.print("gc removeThread: {d}\n", .{lists[threadCycle].items.len});
		...
		for(&lists,0..) |*list,i| {
			if(false)
				std.debug.print("gc removeThread: B{d}\n", .{i});
			
			freeItemsFromList(list);
			list.deinit(main.globalAllocator);
		}
				//std.debug.print("gc removeThread: C\n", .{});
	}
	...
	pub fn deferredFree(item: FreeItem) void {
		//std.debug.print("gc deferredFree: {d}\n", .{threadCycle});
