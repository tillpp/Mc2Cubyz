...
pub const ListUnmanaged = @import("list.zig").ListUnmanaged;
pub const MultiArray = @import("list.zig").MultiArray;
...
pub const Compression = struct { // MARK: Compression
	...
	pub fn pack(sourceDir: files.Dir, writer: anytype) !void {
	...
	pub fn unpack(outDir: files.Dir, input: []const u8) !void {
...
pub fn AliasTable(comptime T: type) type { // MARK: AliasTable
	return struct {
		...
		pub fn sample(self: *const @This(), seed: *u64) *T {
			const initialIndex = random.nextIntBounded(u16, seed, @as(u16, @intCast(self.items.len)));
			if(random.nextInt(u16, seed) < self.aliasData[initialIndex].chance) {
	...
	fn run(self: *ThreadPool, id: usize) void {
		...
		outer: while(true) {
			...
			if(id == 0 and std.time.milliTimestamp() -% lastUpdate > refreshTime) {
				var temporaryTaskList = List(Task).init(Main.stackAllocator);
...
var dynamicIntArrayAllocator: heap.PowerOfTwoPoolAllocator(chunk.chunkVolume/@bitSizeOf(u8), chunk.chunkVolume*@sizeOf(u16), 64) = undefined;
...
pub fn PaletteCompressedRegion(T: type, size: comptime_int) type { // MARK: PaletteCompressedRegion
	...
	return struct {
		...
		pub fn deferredDeinit(self: *Self) void {
			heap.GarbageCollection.deferredFree(.{.ptr = self.impl.raw, .freeFunction = castFunctionSelfToAnyopaque(privateDeinit)});
		}
...
const ReadWriteTest = struct {
	fn getWriter() BinaryWriter {
		return .init(heap.testingAllocator);
	}
...
test "read/write Vec3i" {
	try ReadWriteTest.testVec(vec.Vec3i, .{0, 0, 0});
	try ReadWriteTest.testVec(vec.Vec3i, .{
		std.math.maxInt(@typeInfo(vec.Vec3i).vector.child),
		std.math.minInt(@typeInfo(vec.Vec3i).vector.child),
		std.math.minInt(@typeInfo(vec.Vec3i).vector.child),
	});
	try ReadWriteTest.testVec(vec.Vec3i, .{
		std.math.minInt(@typeInfo(vec.Vec3i).vector.child),
...
test "read/write Vec3f/Vec3d" {
	inline for([_]type{vec.Vec3f, vec.Vec3d}) |vecT| {
...
pub fn SparseSet(comptime T: type, comptime IdType: type) type { // MARK: SparseSet
	...
	return struct {
		...
		dense: ListUnmanaged(T) = .{},
		denseToSparseIndex: ListUnmanaged(IdType) = .{},
		sparseToDenseIndex: ListUnmanaged(IdType) = .{},
...
test "SparseSet/set at zero" {
	...
	defer set.deinit(heap.testingAllocator);

	const index: IdType = @enumFromInt(0);

	set.set(heap.testingAllocator, index, 5);
...
test "SparseSet/remove first" {
	...
	set.set(heap.testingAllocator, firstId, 5);
	set.set(heap.testingAllocator, secondId, expectSecond);
...
test "SparseSet/remove last" {
	...
	set.set(heap.testingAllocator, @enumFromInt(0), 5);
...
test "SparseSet/reusing" {
	...
	set.set(heap.testingAllocator, firstId, expectNew);
