...
pub const ZonElement = union(enum) { // MARK: Zon
	...
	pub fn arraySize(self: *const ZonElement) usize {
		if(self.* != .array) {
			return 0;
		} else {
	...
	fn createElementFromRandomType(value: anytype, allocator: std.mem.Allocator) ZonElement {
		switch(@typeInfo(@TypeOf(value))) {
			...
			.vector => {
				...
				const result = initArray(heap.NeverFailingAllocator{.allocator = allocator, .IAssertThatTheProvidedAllocatorCantFail = {}});
...
test "element parsing" {
	var wrap = heap.ErrorHandlingAllocator.init(std.testing.allocator);
